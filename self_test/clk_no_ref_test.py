...
class clk_no_ref_test(LiteXModule):
	def __init__(self, platform, test_clock_domain):
		self.test_en = CSRStorage(size=1, reset=0,
								  description= "1 - enable test, 0 - disable test")
		self.test_cnt = CSRStatus(size=16,
		...
		platform.add_source("./gateware/LimeDFB/self_test/clk_no_ref_test.vhd")
		...
		# Assign ports
		self.param_ios.update(
			i_clk = ClockSignal(test_clock_domain),
